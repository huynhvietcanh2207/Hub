...
@section('content')
    <div x-data="{
            ...
            <div class="bg-white/35 backdrop-blur-2xl border-t-0 border-l-0 border-r-0 rounded-none shadow-sm"
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between h-16 sm:h-20">
                        ...
                        <a href="{{ route('hub.index') }}" class="flex items-center gap-3 sm:gap-4 hover:opacity-90 active:scale-98 transition-all duration-200">
                            @if($companyLogo)
                            ...
                            @else
                                <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl overflow-hidden
                        ...
                        @if($isAdmin)
                            <div class="flex items-center gap-2">
                                <a href="{{ route('hub.index') }}" class="flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl
                                    ...
                                    <span class="hidden sm:inline text-xs font-medium text-gray-500 group-hover:text-gray-700 transition-colors duration-300">Exit Admin</span>
                                </a>
                                ...
                                <button @click="showSettingsModal = true" id="btn-settings" class="flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl
                                    ...
                                    <span class="hidden sm:inline text-xs font-medium text-gray-500 group-hover:text-gray-700 transition-colors duration-300">Settings</span>
                                </button>
                            </div>
                        @endif
        ...
        <div class="flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 pt-20 pb-12 sm:pb-16 min-h-screen">
            ...
            {{-- ── GLASS CONTAINER ── --}}
            <div class="w-full max-w-xs sm:max-w-xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl animate-fade-in">
                                ...
                                <a href="{{ $website->url }}" target="_blank" rel="noopener noreferrer"
                                    ...
                                    @if($isAdmin)
                                        <form action="{{ route('websites.destroy', $website) }}" method="POST" class="absolute top-1.5 right-1.5 sm:top-2 sm:right-2
                                                     opacity-0 group-hover:opacity-100
                                                     transition-all duration-200 z-10" onclick="event.stopPropagation();">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    @else
                        <div class="overflow-x-auto rounded-xl border border-gray-200/30">
                            <table class="min-w-full divide-y divide-gray-200/20 text-left text-xs sm:text-sm">
                                <thead class="bg-white/40 text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                    <tr>
                                    ...
                                    @foreach($registrations as $reg)
                                        <tr class="hover:bg-white/20 transition-colors duration-150">
                                            ...
                                            <td class="px-4 py-3.5">
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium bg-green-500/10 text-green-700">
                                                    {{ $reg->site_name }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-3.5 text-gray-400 text-[10px] sm:text-xs">{{ $reg->created_at->format('Y-m-d H:i') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
        ...
        <div x-show="showAddModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div x-show="showAddModal" x-transition:enter="transition ease-out duration-300"
                ...
                {{-- Form --}}
                <form action="{{ route('websites.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3.5 sm:space-y-4">
                    @csrf
                    ...
                    <div x-data="{ iconType: 'file' }">
                        ...
                        <div x-show="iconType === 'file'" class="animate-fade-in" style="animation-duration: 200ms;">
                            <input type="file" id="website-icon-file" name="icon_file" accept="image/*"
                        ...
                        <div x-show="iconType === 'url'" class="animate-fade-in" style="animation-duration: 200ms;">
                            <input type="text" id="website-icon-url" name="icon_url" placeholder="https://example.com/icon.png"
                                   value="{{ old('icon_url') }}"
